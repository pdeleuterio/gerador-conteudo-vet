...
// O calendário completo de 365 dias de conteúdo
const calendar = {
 ...
 * @param date A data para a qual buscar a ideia.
 * @returns A string com a ideia do post.
 */
 ...
 * @param tone O tom da comunicação (ex: 'Divertido').
 * @returns A string descrevendo o estilo de imagem.
 */
...
async function handleGenerateContent(req: NextApiRequest, res: NextApiResponse) {
    try {
        ...
        **Tom da Comunicação:** ${tone}
        ...
        - "caption": Uma legenda completa e envolvente para o post. Use emojis de forma natural. Não use formatação markdown (como ** para negrito).
        ...
        const response = await ai.models.generateContent({
            ...
            config: {
                ...
                responseSchema: {
                    type: Type.OBJECT,
                    properties: {
                        caption: { type: Type.STRING, description: "Legenda para o post, sem markdown." },
        ...
        // Garante que a legenda não tenha negrito e adiciona a ideia do post à resposta final
        const finalResponse = {
            ...jsonData,
            post_idea: postIdea,
            caption: jsonData.caption.replace(/\*\*/g, '') // Remove ** do negrito
        };
...
async function handleSearchImages(req: NextApiRequest, res: NextApiResponse) {
    try {
        ...
        // Adiciona termos focados em pets para garantir a relevância
        const petFocusedSearchTerm = `${searchTerm} pet animal`;
        ...
        unsplashApiUrl.searchParams.append('orientation', 'squarish');
